/* L3/L4: many green threads run across multiple worker cores, hammering a
 * shared atomic counter with yields in between. Correct final total proves the
 * scheduler runs and resumes tasks across workers without losing or duplicating
 * them; TSAN validates the cross-worker happens-before on resume. */
...
int main(void) {
    ...
    dlsm_gt_runtime *rt = dlsm_gt_runtime_new(4, 0); /* 4 worker cores */
    if (!rt) { fprintf(stderr, "runtime_new failed\n"); return 1; }
    for (int i = 0; i < NTASKS; i++) { dlsm_gt_spawn(rt, worker_task, NULL); }
    dlsm_gt_run(rt);
    ...
    printf("gt parallel ok: %ld increments across %d tasks / 4 workers\n", got, NTASKS);
    return 0;
}
